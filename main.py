...
class SmartNotes(QMainWindow):
    def   __init__(self):
        ...
        self.ui.lineEdit.textChanged.connect(self.search_note)
    ...
    def load_note(self):
        ...
        self.show_list(self.notes)

    def show_list(self, notes):
        for title, note in notes.items():
            item = QListWidgetItem(title)
            if "datetime" in note:
                tip = f"Збережено: {note['datetime']}"
                item.setToolTip(tip)
            self.ui.note_list.addItem(item)
    ...
    def search_note(self):
        search = self.ui.lineEdit.text().strip().lower()
        result = {}
        if search != "":
